...
import { ALL_PAIRS_SERIES_CAP, CHROME, type Mode, seriesColor } from "../lib/palette";
...
export default function ChartView({ spec, columns, rows, truncated, mode }: Props) {
  ...
  // A single series is named by the title; a legend box would repeat it.
  const legend =
    spec.y.length > 1 ? (
      <Legend
  ...
  // Scatter reads every pair at once; the shared cap applies to the rest too,
  // and the backend already limits y to three measures.
  const measures = spec.y.slice(0, ALL_PAIRS_SERIES_CAP);
